    ...
    try:
        with get_session() as db:
            ...
            active_waitlist = (
                ...
                .join(PatientContact)
                .filter(WaitlistEntry.active == True, PatientContact.opt_out == False)
                .count()
            )
...
def show_offer_card(offer: Offer, db: Session):
    ...
    with col1:
        ...
        waitlist_entry = (
            db.query(WaitlistEntry)
            .filter(WaitlistEntry.patient_id == offer.patient_id, WaitlistEntry.active == True)
            .first()
        )
    ...
    try:
        with get_session() as db:
            ...
            providers = (
                ...
                .filter(ProviderReference.active == True)
                    ...
                    if not location:
                        ...
                        try:
                            ...
                            with get_session() as add_db:
                                ...
                                st.success(
                                    f"✅ Cancellation created successfully! (ID: {cancellation.id})"
                                )
                                st.info(
                                    "🤖 The orchestrator will automatically send offers to the top waitlist candidates within moments."
                                )
                                ...
                                st.write(
                                    f"- Time: {slot_start.strftime('%b %d, %Y at %I:%M %p')} CT"
                                )
                ...
                try:
                    with get_session() as add_db:
                        ...
                        existing_entry = (
                            ...
                            .filter(
                                WaitlistEntry.patient_id == patient.id, WaitlistEntry.active == True
                            )
                            .first()
                        )
        ...
        try:
            with get_session() as db:
                active_entries = (
                    db.query(WaitlistEntry)
                    .join(PatientContact)
                    .filter(WaitlistEntry.active == True)
                    .order_by(desc(WaitlistEntry.priority_score))
                    .all()
                )
