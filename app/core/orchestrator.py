...
class OfferOrchestrator:
    ...
    def send_next_batch(self, cancellation_id: int) -> int:
        ...
        for entry in eligible_entries:
            ...
            try:
                status_callback = (
                    self._get_status_callback_url() if settings.TWILIO_WEBHOOK_BASE_URL else None
                )

                twilio_sid = twilio_client.send_sms(
                    to=patient.phone_e164, body=message_body, status_callback=status_callback
                )
    ...
    def handle_patient_acceptance(self, from_phone: str, message_body: str) -> tuple[bool, str]:
        ...
        offer = (
            ...
            .filter(and_(Offer.patient_id == patient.id, Offer.state == OfferState.PENDING))
    ...
    def check_expired_holds(self) -> int:
        ...
        expired_offers = (
            self.session.query(Offer)
            .filter(and_(Offer.state == OfferState.PENDING, Offer.hold_expires_at <= now))
            .all()
        )
