...
def format_initial_offer(
    slot_time: datetime, location: str, provider_name: str, hold_minutes: int = 7
) -> str:
    """
...
def format_acceptance_winner(slot_time: datetime, location: str, provider_name: str) -> str:
    """
...
def format_welcome_message(
    patient_name: str | None = None, current_appt_date: datetime | None = None
) -> str:
    """
...
def parse_patient_response(message_body: str) -> str | None:
    """
    ...
    # YES variations
    if any(
        keyword in body for keyword in ["YES", "Y", "YEAH", "YEP", "OK", "OKAY", "SURE", "ACCEPT"]
    ):
        return "YES"
    ...
    if any(
        keyword in body for keyword in ["STOP", "UNSUBSCRIBE", "CANCEL", "END", "QUIT", "REMOVE"]
    ):
        return "STOP"
